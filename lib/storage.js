...
function getSubmissionById(id) {
  ...
  const match = fs.readdirSync(SUBMISSIONS_DIR).find((f) => f.endsWith(`--${short}.json`));
  if (!match) return null;
  return JSON.parse(fs.readFileSync(path.join(SUBMISSIONS_DIR, match), "utf8"));
}

function submissionToCsv(record) {
  return (
    toCsvRow(CSV_COLUMNS.map(([col]) => col)) +
    toCsvRow(CSV_COLUMNS.map(([, get]) => get(record)))
  );
}
